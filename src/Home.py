...
st.markdown("Our team is working to understand and inform others about the deaths of migrants, focused at borders. We are relying on the [Missing Migrants Project](https://missingmigrants.iom.int/) dataset, which includes data from 2014 to 2022. This data includes incidents across the world, each dot on the map representing the death of 1 or more persons, with details about the location, cause of death, time, number of survivors, and more, if it can be identified.  In this project, we use this data to create an interactive data visualization world map, allowing readers to see how migrant deaths vary by location, cause of death, gender, and season.")
st.markdown("To highlight that the occurrence of these tragedies is an ongoing phenomenon, we wanted to add new data from the most recent reports to our dataset. We achieved this by pulling Tweets from different reputable news sources on Twitter. This part can be found by visiting the Twitter page in the sidebar.")
...
st.markdown("We welcome you to explore the below visualization by zooming into the map directly or by using the filters in the left side menu. First, let's explore some global statistics.")
